...
function Navbar() {
	...
	return (
		<nav className="sticky top-0 z-10 flex items-center justify-between py-4 border-b-[1px] backdrop-blur-lg dark:border-slate-50 border-slate-400 dark:border-opacity-10 border-opacity-20">
			...
			{/* Navbar Links */}
			<div
				className={`fixed lg:flex lg:relative lg:flex-row gap-4 lg:gap-10 pt-6 lg:py-0 w-full lg:items-center lg:justify-between lg:w-fit flex flex-col px-sectionpl h-screen lg:h-fit dark:bg-base bg-white z-[1] ${
					isOpen ? "top-[73px] lg:top-0" : "-left-full"
				}`}
			>
				{menuOptions.map((option, index) => (
					<div
						className="w-full transition-all duration-500 ease-in-out lg:flex lg:items-center lg:w-fit"
						key={index}
					>
						<a
